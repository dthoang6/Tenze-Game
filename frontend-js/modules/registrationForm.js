...
export default class RegistrationForm {
  ...
  //methods
  //check if there a no errors to handle the form submit
  formSubmitHandler() {
  ...
  //compare the keystroke different value, ignore no value keystroke like shift
  ...
  //this method will run after every keystroke that changes the username field's value.
  usernameHandler() {
    this.username.errors = false; //assuming there are no errors
    //our validation checks that run immediately for each keystroke value
    this.usernameImmediately();
    //our validation checks after delay
    clearTimeout(this.username.timer);
    this.username.timer = setTimeout(() => this.usernameAfterDelay(), 800);
  }

  passwordHandler() {
    this.password.errors = false; //assuming there are no errors
    //our validation checks that run immediately for each keystroke value
    this.passwordImmediately();
    //our validation checks after delay
  ...
  emailHandler() {
    this.email.errors = false; //assuming there are no errors
    //our validation checks after delay
  ...
  usernameImmediately() {
    //we want to check for something that is not allow
    ...
    //if there are no errors, hide the red error message
  ...
  showValidationError(el, message) {
    ...
    el.errors = true; //there is an error
  }
  ...
  usernameAfterDelay() {
    ...
    //using axios to send async to server to check for username existence
    axios
      .post("/doesUsernameExist", { username: this.username.value, _csrf: this._csrf })
      .then(response => {
        //the response is true which the username is already exist.
        if (response.data) {
